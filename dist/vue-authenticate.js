...
(function (global, factory) {
	...
	(factory((global['vue-authenticate'] = global['vue-authenticate'] || {})));
...
var LocalStorage = function () {
  ...
  createClass(LocalStorage, [{
    ...
    key: '_getStorageKey',
    value: function _getStorageKey(key) {
      if (this.namespace) {
...
var MemoryStorage = function () {
  function MemoryStorage(namespace) {
...
var OAuthPopup = function () {
  function OAuthPopup(url, name, popupOptions) {
    classCallCheck(this, OAuthPopup);
  ...
  createClass(OAuthPopup, [{
    key: 'open',
    value: function open(redirectUri, skipPooling) {
      try {
    ...
    key: 'pooling',
    value: function pooling(redirectUri) {
      var _this = this;
    ...
    value: function _stringifyOptions() {
      var options = [];
...
var OAuth = function () {
  function OAuth($http, storage, providerConfig, options) {
  ...
  createClass(OAuth, [{
    ...
    value: function init(userData) {
      ...
      return this.getRequestToken().then(function (response) {
        return _this.openPopup(response).then(function (popupResponse) {
    ...
    key: 'openPopup',
    value: function openPopup(response) {
    ...
    value: function exchangeForToken(oauth, userData) {
...
var OAuth2 = function () {
  function OAuth2($http, storage, providerConfig, options) {
  ...
  createClass(OAuth2, [{
    ...
    value: function init(userData) {
      ...
      return new Promise(function (resolve, reject) {
        _this.oauthPopup.open(_this.providerConfig.redirectUri).then(function (response) {
          if (_this.providerConfig.responseType === 'token' || !_this.providerConfig.url) {
            return resolve(response);
          }

          if (response.state && response.state !== _this.storage.getItem(stateName)) {
            return reject(new Error('State parameter value does not match original OAuth request state value'));
          }

          resolve(_this.exchangeForToken(response, userData));
        }).catch(function (err) {
          reject(err);
        });
      });
    ...
    key: '_stringifyRequestParams',
    value: function _stringifyRequestParams() {
      var _this2 = this;
      ...
      paramCategories.forEach(function (categoryName) {
        if (!_this2.providerConfig[categoryName]) return;
        if (!Array.isArray(_this2.providerConfig[categoryName])) return;

        _this2.providerConfig[categoryName].forEach(function (paramName) {
          var camelCaseParamName = camelCase(paramName);
          var paramValue = isFunction(_this2.providerConfig[paramName]) ? _this2.providerConfig[paramName]() : _this2.providerConfig[camelCaseParamName];

          if (paramName === 'redirect_uri' && !paramValue) return;

          if (paramName === 'state') {
            var stateName = _this2.defaults.name + '_state';
            paramValue = encodeURIComponent(_this2.storage.getItem(stateName));
          }
          if (paramName === 'scope' && Array.isArray(paramValue)) {
            paramValue = paramValue.join(_this2.providerConfig.scopeDelimiter);
            if (_this2.providerConfig.scopePrefix) {
              paramValue = [_this2.providerConfig.scopePrefix, paramValue].join(_this2.providerConfig.scopeDelimiter);
            }
          }
...
var VueAuthenticate = function () {
  function VueAuthenticate($http, overrideOptions) {
    var _this = this;

    classCallCheck(this, VueAuthenticate);
    ...
    if (this.options.bindRequestInterceptor && isFunction(this.options.bindRequestInterceptor)) {
      ...
      this.$http.interceptors.push(function (request, next) {
        if (_this.isAuthenticated()) {
          request.headers.set('Authorization', [_this.options.tokenType, _this.getToken()].join(' '));
        } else {
          request.headers.delete('Authorization');
        }

        if (_this.options.bindResponseInterceptor && isFunction(_this.options.bindResponseInterceptor)) {
          _this.options.bindResponseInterceptor.call(_this);
        } else {
          next(function (response) {
            try {
              var responseJson = JSON.parse(response[_this.options.responseDataKey]);
              if (responseJson[_this.options.tokenName]) {
                _this.setToken(responseJson);
                delete responseJson[_this.options.tokenName];
                return responseJson;
              }
            } catch (e) {}
            return response;
  ...
  createClass(VueAuthenticate, [{
    ...
    key: 'login',
    value: function login(user, requestOptions) {
      var _this2 = this;
    ...
    key: 'register',
    value: function register(user, requestOptions) {
      var _this3 = this;
    ...
    key: 'logout',
    value: function logout(requestOptions) {
      var _this4 = this;
      ...
      if (requestOptions.url) {
        ...
        return this.$http(requestOptions).then(function (response) {
          _this4.storage.removeItem(_this4.tokenName);
        });
      } else {
    ...
    key: 'authenticate',
    value: function authenticate(provider, userData, requestOptions) {
      var _this5 = this;

      return new Promise$1(function (resolve, reject) {
        var providerConfig = _this5.options.providers[provider];
        if (!providerConfig) {
        ...
        switch (providerConfig.oauthType) {
          case '1.0':
            providerInstance = new OAuth(_this5.$http, _this5.storage, providerConfig, _this5.options);
            break;
          case '2.0':
            providerInstance = new OAuth2(_this5.$http, _this5.storage, providerConfig, _this5.options);
            break;
          default:
...
exports.VueAuthenticate = VueAuthenticate;

Object.defineProperty(exports, '__esModule', { value: true });

})));
//# sourceMappingURL=vue-authenticate.js.map
